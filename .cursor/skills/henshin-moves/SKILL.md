...
改特效、招式或数值前先扫一眼上表对应行；缺项就地补进权威文档，不要只写在聊天里。改共享 `HomingAI` 时读需求 §6：广角镜圆形 8/16/32、60° 第一锁、断锁 2×；**禁止**把广角镜退回全向「圈里最近」。招式自带索敌（精神强念、念力等）走另一路全向掉头，**不要**叠广角镜锥角/短半径；细节以 `docs/move-effects.md` 为准。
