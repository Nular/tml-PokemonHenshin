...
**现役代码（2026-09-11）：** 36 形态 + A01–A28（碎片/普通/超级）；被动+技能1/2+能量大招；**v1.4 已接线**（含龙之波动爆炸碎片命中 `1×Factor`；广角镜按档圆形索敌、自带追踪不叠饰品见需求 §6）；世界档/`100` **只挡获取经验**（不截存档；卡顶/满级不飘 `EXP +X`）；**神奇糖果**已接线；御三家建角发放 + 旧档补发（需求 §4.4，进世界已本地验收）；属性面板在图鉴按钮下方；变身地图头像用形态全身图缩小填入原版头像框（需求 §2.3）。联机瞄准（`SyncAim`）与壳弹旁观端贴图重建**已接线**，双端验收 pending。游戏内 DPS 抽检、联机双端（含地图头像/虫洞药水）、饰品图标/合成/掉落/索敌与糖果验收仍待本地。
