...
**现役代码（2026-09-05）：** 36 形态 + **A01～A21**；被动+技能1/2+能量大招（默认 Mouse3）；能量 UI（Tooltip / 物品底栏 / 右下角条）；点名招式特效已重做（见 `docs/move-effects.md` v1.2）。

**待游戏内验收：** Build + Reload 后验手感/特效/联机；调试 `/henshin stage`、`/henshin evolve`。  
**注意：** 御三家二阶进度看 **史莱姆神 / 鹿角怪**，不是史莱姆王。

后置：未点名大招精修、联机双端实测、DPS 对标、Rage/肾上腺素、M5 发布打磨。
...
- 特效：不新增 FX 图片；点名招式以原版贴图/尘为主；可参考灾厄/CalamityOverhaul 写法（不运行时依赖 CWR）
