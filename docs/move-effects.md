...
| FormId | Name | Passive (Terraria) | Skill1 | Skill2 | Ultimate | Energy/aftermath | VFX note | Net risk |
|--------|------|-------------------|--------|--------|----------|------------------|----------|----------|
| L08_F01 | 金属怪 | **恒净之躯**：免疫本模关注的 debuff（着火/毒/感电/缓速等可列白名单） | 念力 Bolt+Stun | 臂锤 下砸拳头（Kenney fightFist） | 金属爪 鼠标双侧镜像 scratch 渐进+持续伤≈16格 | 耗 100 | `Assets/Fx/Kenney` fist/scratch CC0 | Low | Done |
...
| FormId | Name | Passive (Terraria) | Skill1 | Skill2 | Ultimate | Energy/aftermath | VFX note | Net risk |
|--------|------|-------------------|--------|--------|----------|------------------|----------|----------|
| L04_F01 | 皮卡丘 | **静电**：被敌对接触/近战打中时，反弹 Electrify（短 CD） | 电击 Bolt+Electrify **5s**（**撞墙**；戴电气球：弹速+100%/+200%、穿3、加宽） | 电光一闪 Blink（指针最近敌；落点电爆伤；2s CD + 0.25s 无敌；一屏；电气球冷却 -20%/-40%） | 十万伏特：向前曲折天雷（≤64 格）；50% Electrify；命中后 16 格内最多 **三叉**（分叉不再分）；0.5s 后对感电敌再射（不再分叉）；广角镜软瞄光标 4/8 格 | 耗满；大招 ×**4.8**（§7.1） | `SkyBoltLightning` + 分叉 `ai2=2` | Medium | Done |
| L04_F02 | 雷丘 | **静电**：同上 | 十万伏特 SkyBolt 向鼠标（≤**128** 格；穿怪；**不**挂感电二次；命中三叉同上；广角镜软瞄） | 伏特攻击：闪现到鼠标 + Recoil + i 帧；加粗闪电拖尾 60tick、面板×**3.0**、同怪最多 **6** 段 + 形态残影 | 打雷：屏内每敌头上一道更粗落雷 | 耗 100；伏特 Recoil | S1=`ThunderboltSkill`；拖尾=`VoltTackleTrail`；大招落雷 ai0=1 | Medium | Done |
