...
- F5 更高一档矿 **或** 下一档 Boss。
- F6 再下一档 Boss / 晚事件。
- 掉率（非合成）：匣 **10%**；事件小怪 **2%**（每只，需 `player.RollLuck`）；Boss **25%**（专家袋可再 Roll 一次，用 `npc.boss` 与 `NPC.GetBossHeadTextureIndex` 或 `IsBossForXp`）。多人：按击杀归属玩家掉在 Boss 处（`player.whoAmI` closest / `npc.lastInteraction`）。
